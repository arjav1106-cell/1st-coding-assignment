...
x_t[object_cols] = ordinal_encoder.fit_transform(x_train[object_cols])
x_v[object_cols] = ordinal_encoder.transform(x_val[object_cols])

# the number of rows in x_t and y_train (also x_v and y_val) are not same so making them same 
y_t = y_train.loc[x_t.index]
y_v = y_val.loc[x_v.index]

# printing the MAE
print("The MAE from Ordinal Encoding method is:")
print(f"{scores(x_t, x_v, y_t, y_v):.2f}")
